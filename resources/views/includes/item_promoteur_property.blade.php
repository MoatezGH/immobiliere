<div class="product-item">
    <div class="product-img">
        ...
        @if ($item->getFirstImageOrDefault())
            <img src="{{ asset('uploads/promoteur_property/' . $item->getFirstImageOrDefault()) }}" alt="{{ $item->title }}">
        @else
        ...
        <div class="product-top">
            <div class="product-category">
                ...
                <h6 class="product-category-title"><a
                        href="#">{{ ucfirst($item->user->promoteur->first_name . ' ' . $item->user->promoteur->last_name) }}
                    </a></h6>
            </div>
...
"><img src="{{ asset('icon_png/location.png') }}"
                    alt="{{ $item->city->name }} ,
                {{ $item->area->name }}" style="width:55px;margin-left: -9px;">
                {{ $item->city->name }} ,
                {{ $item->area->name }}</p>
...
"><img src="{{ asset('icon_png/m22.png') }}" alt="m²"
                ...
                {{ $item->surface_totale }} (m²)</div>


            <div class="product-date" style="
    margin-top: -12px;
    margin-left: -4px;
"> <img src="{{ asset('icon_png/calendar.png') }}" alt="{{ $item->created_at }}"
                    style="width:55px;margin-left: -9px;">
                {{ $item->created_at }}</div>
        </div>
        <div class="product-bottom">
            ...
            <a href="{{ route('prop_info_promoteur', $item->slug) }}" class="product-text-btn" style="font-size:12px">Voir Détails <i
