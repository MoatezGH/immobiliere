            ...
            <div class="form-group-icon">
                <select class="select" name="city_id" id="citySelect">
                    ...
                    @foreach ($cities as $item)
                        <option value="{{ $item->id }}" {{ request()->city_id == $item->id ? 'selected' : '' }} data-id=" {{ $item->id }}">
                            {{ $item->name }}</option>
                    @endforeach
    ...
    <div class="col-12 mb-1">
        <div class="sidebar-category">
            {{-- @foreach ($categories as $category)
                                                <a href=""><i class="far fa-home"></i> {{ ucfirst($category->name) }}
                                                    <span></span></a>
                                            @endforeach --}}
            <div class="form-group-icon">
    ...
    @foreach ($operations as $item)
        <div class="col-12 mb-2">
            <div class="form-check">
                <input class="form-check-input" name="operation_id" type="radio" value="{{ $item->id }}"
                    {{ request()->operation_id == $item->id ? 'checked' : '' }}>
